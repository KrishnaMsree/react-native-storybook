...
export default function RetirementOutLook({BtnText,color}){
    ...
    return (
        ...
        <View style={globalStyles.viewContainer}>
            ...
            <Card>
            {
                datas.map(d => {
                    return (
                        <View key={d.key}>
                            <Text>{d.name}</Text>
                            <Text style={{fontWeight:'bold',marginBottom:15,color:color}}>{d.val}</Text>
                        </View>
                    )
                })
            }
            ...
            <View style={{marginTop:10}}>
                <Buttons  text={BtnText.text} onPress={()=>console.log(`${BtnText.text} button pressed`)}/>
